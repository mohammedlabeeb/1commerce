...
OutputItemPromoCode   = 'PROMOTIONCODE_';  // Promotion Code
OutputItemDebug       = 'DEBUG_'           // Programmer Debug
...
if ( !bLanguageDefined ) {
   ...
   strErrPromo = "Invalid Promotion";
   strNewQty = 'Please enter new quantity:';
   bLanguageDefined = true;
}
//for some inexplicable reason unless it's defined below strErrPromo will not assign - beats me
var strPromLabel = "Promotion";
var strErrPromo = "Invalid Promotion";
var debug = "";
...
//---------------------------------------------------------------------||
function CKquantity(checkString, allow_blank) {
   var strNewQuantity = "";
   ...
   if ( strNewQuantity.length < 1 ){
   		if(allow_blank == 1){strNewQuantity = "";}else{strNewQuantity = "1";}
   }
...
//---------------------------------------------------------------------||
function AddToCart(thisForm, sell_exVAT) {
   ...
   if ( thisForm.PRICE_EXVAT == null )
      strPRICE_EXVAT     = "0.00";
   else
      strPRICE_EXVAT     = thisForm.PRICE_EXVAT.value;
	  
   if ( thisForm.BASE_PRICE == null )
      strBASE_PRICE     = "0.00";
   else
      strBASE_PRICE     = thisForm.BASE_PRICE.value;
   ...
   if ( thisForm.ADDITIONALINFO == null ) {
      strADDTLINFO = "";
			...
			if(sell_exVAT == "N"){
				...
				strPRICE_EXVAT = precise_round(((parseFloat(strPRICE) * 10000) / ((10000 + parseFloat(strTAX) * 100)/100))/100, 2);
			}else{
				//Sell using exVAT in which case any adjustments will be to the exVAT value
				strPRICE_EXVAT = parseFloat(strPRICE_EXVAT) + parseFloat(xx[1]);
				strPRICE = addVAT(strPRICE_EXVAT, strTAX)
			}
			if(xx[2].length > 0){
				if (strSKU.length > 0){
					strSKU = strSKU + "-" + xx[2];
   ...
   if ( thisForm.PROMO_CODE_VALID == null )
      strPROMO_CODE_VALID    = "";
   else
      strPROMO_CODE_VALID    = thisForm.PROMO_CODE_VALID.value;

   if ( thisForm.PROMO_CODE_USER == null )
      strPROMO_CODE_USER    = "";
   else
      strPROMO_CODE_USER    = thisForm.PROMO_CODE_USER.value;
	  
   if ( thisForm.PROMO_TYPE == null )
      strPROMO_TYPE    = "";
   else
      strPROMO_TYPE    = thisForm.PROMO_TYPE.value;
	  
   if ( thisForm.PROMO_ADJUST == null )
      strPROMO_ADJUST    = "";
   else
      strPROMO_ADJUST    = thisForm.PROMO_ADJUST.value;
   //add Quantity Discount fields  
   if ( typeof thisForm.QD_TYPE == "undefined" )
      strQD_TYPE    = "";
   else
      strQD_TYPE    = thisForm.QD_TYPE.value;
   ...
   for ( i = 1; i <= iNumberOrdered; i++ ) {
      ...
      fields[2] = database.substring( Token1+1, Token2 );			// Price - including VAT
	  ...
	  fields[11] = database.substring( Token10+1, Token11 );	// SCurrency Symbol
	  MonetarySymbol = fields[11];
	  //Promotions
	  fields[12] = database.substring( Token11+1, Token12 );			// Promotion Code - valid code
	  fields[13] = database.substring( Token12+1, Token13 );			// Promotion Code - User Keyed
	  fields[14] = database.substring( Token13+1, Token14 );			// Promotion Type
	  fields[15] = database.substring( Token14+1, Token15 );			// Promotion Adjustment
	  //Quantity Discounts
	  fields[16] = database.substring( Token15+1, Token16 );			// Quantity Discount Type
	  fields[17] = database.substring( Token16+1, Token17 );			// Price - EXVat - of the original Base Price
	  fields[18] = database.substring( Token17+1, database.length );	// Base Price - including VAT - for Quantity Discount calculations
	  																   // The Base Price is the original selling price before any QD's are applied
      ...
      if ( fields[0] == strID_NUM &&
         ...
         bAlreadyInCart = true;
		 //at this point recalculate any quantity discount based on the latest quantity
		 if(strQD_TYPE != ""){
			//Quantity Discount has been applied so we need to calculate the selling and exVAT prices
			var strPRICE = getCartQDPrice(strQD_TYPE, strID_NUM, (parseInt(strQUANTITY)+parseInt(fields[1])), strPRICE, strPRICE_EXVAT, strBASE_PRICE, strTAX);
			var strPRICE_EXVAT = precise_round(((parseFloat(strPRICE) * 10000) / ((10000 + parseFloat(strTAX) * 100)/100))/100, 2);
		 }
   ...
   if ( !bAlreadyInCart ) {
      ...
      if ( iNumberOrdered > 75 )
      {
		 ...
		 if(strQD_TYPE != ""){
			//Quantity Discount has been applied so we need to calculate the selling and exVAT prices
			var strPRICE = getCartQDPrice(strQD_TYPE, strID_NUM, strQUANTITY, strPRICE, strPRICE_EXVAT, strBASE_PRICE, strTAX);
			var strPRICE_EXVAT = precise_round(((parseFloat(strPRICE) * 10000) / ((10000 + parseFloat(strTAX) * 100)/100))/100, 2);
		 }
         dbUpdatedOrder = strID_NUM    + "|" + 
                          strQUANTITY  + "|" +
...
// PARAMETERS:  Form Object                                            ||
// RETURNS:     Cookie to user's browser, with prompt                  ||
// PURPOSE:     Adds all products in table to the user's shopping      ||
//              cart                                                   ||
//---------------------------------------------------------------------||
function AddTableToCart(thisForm, sell_exVAT) {

    if (isDropShowing()){
        return false;
    }
	
   //check at least quantity has been input otherwise error
   var iNumberOrdered = 0; var tableRows = thisForm.ROW_COUNTER.value; var qty_input = 0;
   for( var h=0; h<tableRows; h++){
	   if(eval("thisForm.QUANTITY_" + h) != null && eval("thisForm.QUANTITY_" + h + ".value") != 0){
		   qty_input = 1;
		   break;
	   }
   }
   if(qty_input == 0){
	  notice = "Please enter a quantity against at least 1 line";
	  ShowAlert(notice, "red");
	  //location.href=location.href;
	  return false;
   }
	
   var iNumberOrdered = 0; var tableRows = thisForm.ROW_COUNTER.value;
   for( var h=0; h<tableRows; h++){
	   if(eval("thisForm.QUANTITY_" + h) != null && eval("thisForm.QUANTITY_" + h + ".value") != 0){
		   //only process if a quantity has been input
		   var bAlreadyInCart = false;
		   var notice = "";
		   iNumberOrdered = GetCookie("NumberOrdered");
		   ...
		   if ( eval("thisForm.ID_NUM_" + h) == null )
			  strID_NUM    = "";
		   else
			  strID_NUM    = eval("thisForm.ID_NUM_" + h + ".value");
		   if ( eval("thisForm.SKU_" + h) == null )
			  strSKU    = "";
		   else
			  strSKU    = eval("thisForm.SKU_" + h + ".value");
			  
		   if ( eval("thisForm.CURRENCY_SYMBOL" + h) == null )
			  strCURRENCY_SYMBOL    = "";
		   else
			  strCURRENCY_SYMBOL    = eval("thisForm.CURRENCY_SYMBOL_" + h + ".value");
			 
		   if ( eval("thisForm.QUANTITY_" + h) == null )
			  strQUANTITY  = "1";
		   else
			  strQUANTITY  = eval("thisForm.QUANTITY_" + h + ".value");
			 
		   if ( eval("thisForm.PRICE_" + h) == null )
			  strPRICE     = "0.00";
		   else
			  strPRICE     = eval("thisForm.PRICE_" + h + ".value");
			  
		   if ( eval("thisForm.PRICE_EXVAT_" + h) == null )
			  strPRICE_EXVAT     = "0.00";
		   else
			  strPRICE_EXVAT     = eval("thisForm.PRICE_EXVAT_" + h + ".value");
			  
		   if ( eval("thisForm.BASE_PRICE_" + h) == null )
			  strBASE_PRICE     = "0.00";
		   else
			  strBASE_PRICE     = eval("thisForm.BASE_PRICE_" + h + ".value");			  
			  
		   if ( eval("thisForm.NAME_" + h) == null )
			  strNAME      = "";
		   else
			  strNAME      = eval("thisForm.NAME_" + h +".value");
		
		   if ( eval("thisForm.SHIPPING_" + h) == null )
			  strSHIPPING  = "0.00";
		   else
			  strSHIPPING  = eval("thisForm.SHIPPING_" + h +".value");
			  
		   if ( eval("thisForm.TAX_" + h) == null )
			  strTAX  = "0.00";
		   else
			  strTAX  = eval("thisForm.TAX_" + h + ".value");
			  
		   if ( eval("thisForm.WEIGHT_" + h) == null )
			  strWEIGHT  = "0.00";
		   else
			  strWEIGHT  = eval("thisForm.WEIGHT_" + h + ".value");
			  
		   if ( eval("thisForm.TAXEXEMPTION_" + h) == null )
			  strTAXEXEMPTION  = "0";
		   else
			  strTAXEXEMPTION  = eval("thisForm.TAXEXEMPTION_" + h + ".value");
			  	   
		   if ( eval("thisForm.ADDITIONALINFO_" + h) == null ) {
			  strADDTLINFO = "";
		   } else {
			   //this should never be the case since we shouldn't have any options when in table input mode
			   
					if (eval("thisForm.ADDITIONALINFO_" + h + ".selectedIndex") == 0) {
						notice = "Please " + eval("thisForm.ADDITIONALINFO_" + h + "[thisForm.ADDITIONALINFO_" + h + ".selectedIndex].text") + " from the drop down menu";
						ShowAlert(notice, "red");
						//location.href=location.href;
						return false; }
					var xx = eval("thisForm.ADDITIONALINFO_" + h + "[thisForm.ADDITIONALINFO_" + h + ".selectedIndex].value.split('^')");
					strADDTLINFO = xx[0];
					if(sell_exVAT == "N"){
						strPRICE = parseFloat(strPRICE) + parseFloat( xx[1] );
						strPRICE_EXVAT = precise_round(((parseFloat(strPRICE) * 10000) / ((10000 + parseFloat(strTAX) * 100)/100))/100, 2);
		   ...
		   if ( eval("thisForm.ADDITIONALINFO2_" + h) != null ) {
			   //this should never be the case since we shouldn't have any options when in table input mode
			   
					if (eval("thisForm.ADDITIONALINFO2_" + h + ".selectedIndex") == 0) {
						notice = "Please " + eval("thisForm.ADDITIONALINFO2_" + h + "[thisForm.ADDITIONALINFO2_" + h + ".selectedIndex].text") + " from the drop down menu";
						ShowAlert(notice, "red");
						//location.href=location.href;
						return false; }
					var xx = eval("thisForm.ADDITIONALINFO2_" + h + "[thisForm.ADDITIONALINFO2_" + h + ".selectedIndex].value.split('^')");
					strADDTLINFO += "; " + xx[0];
					if(sell_exVAT == "N"){
						strPRICE = parseFloat(strPRICE) + parseFloat( xx[1] );
						strPRICE_EXVAT = precise_round(((parseFloat(strPRICE) * 10000) / ((10000 + parseFloat(strTAX) * 100)/100))/100, 2);
		   ...
		   if ( eval("thisForm.ADDITIONALINFO3_" + h) != null ) {
			   //this should never be the case since we shouldn't have any options when in table input mode
			   
					if (eval("thisForm.ADDITIONALINFO3_" + h + ".selectedIndex") == 0) {
						notice = "Please " + eval("thisForm.ADDITIONALINFO3_" + h + "[thisForm.ADDITIONALINFO3_" + h + ".selectedIndex].text") + " from the drop down menu";
						ShowAlert(notice, "red");
						//location.href=location.href;
						return false; }
					var xx = eval("thisForm.ADDITIONALINFO3_" + h + "[thisForm.ADDITIONALINFO3_" + h + ".selectedIndex].value.split('^')");
					strADDTLINFO += "; " + xx[0];
					if(sell_exVAT == "N"){
						strPRICE = parseFloat(strPRICE) + parseFloat( xx[1] );
						strPRICE_EXVAT = precise_round(((parseFloat(strPRICE) * 10000) / ((10000 + parseFloat(strTAX) * 100)/100))/100, 2);
		   ...
		   if ( eval("thisForm.ADDITIONALINFO4_" + h) != null ) {
			   //this should never be the case since we shouldn't have any options when in table input mode
			   
					if (eval("thisForm.ADDITIONALINFO4_" + h + ".selectedIndex") == 0) {
						notice = "Please " + eval("thisForm.ADDITIONALINFO4_" + h + "[thisForm.ADDITIONALINFO4_" + h + ".selectedIndex].text") + " from the drop down menu";
						ShowAlert(notice, "red");
						//location.href=location.href;
						return false; }
					var xx = eval("thisForm.ADDITIONALINFO4_" + h + "[thisForm.ADDITIONALINFO4_" + h + ".selectedIndex].value.split('^')");
					strADDTLINFO += " " + xx[0];
					if(sell_exVAT == "N"){
						strPRICE = parseFloat(strPRICE) + parseFloat( xx[1] );
						strPRICE_EXVAT = precise_round(((parseFloat(strPRICE) * 10000) / ((10000 + parseFloat(strTAX) * 100)/100))/100, 2);
		   ...
		   if ( eval("thisForm.USERENTRY_" + h) != null ) {
					strADDTLINFO += " " + eval("thisForm.USERENTRY_" +h + ".value");
		   } 
		   if ( eval("thisForm.USERENTRY2_" + h) != null ) {
					strADDTLINFO += "; Line 2: " + eval("thisForm.USERENTRY2_" + h + ".value");
		   }
		   if ( eval("thisForm.USERENTRY3_" + h) != null ) {
					strADDTLINFO += "; Line 3: " + eval("thisForm.USERENTRY3_" + h + ".value");
		   } 
		   if ( eval("thisForm.USERENTRY4_" + h) != null ) {
					strADDTLINFO += "; Line 4: " + eval("thisForm.USERENTRY4_" + h + ".value");
		   }
		   if ( eval("thisForm.USERENTRY5_" + h) != null ) {
					strADDTLINFO += "; Line 5: " + eval("thisForm.USERENTRY5_" + h + ".value");
		   }
		   if ( eval("thisForm.USERENTRY6_" + h) != null ) {
					strADDTLINFO += "; Line 6: " + eval("thisForm.USERENTRY6_" + h + ".value");
		   }
		   if ( eval("thisForm.USERENTRY7_" + h) != null ) {
					strADDTLINFO += "; Line 7: " + eval("thisForm.USERENTRY7_" + h + ".value");
		   }
		   if ( eval("thisForm.USERENTRY8_" + h) != null ) {
					strADDTLINFO += "; Line 8: " + eval("thisForm.USERENTRY8_" + h + ".value");
		   }
		   if ( eval("thisForm.USERENTRY9_" + h) != null ) {
					strADDTLINFO += "; Line 9: " + eval("thisForm.USERENTRY9_" + h + ".value");
		   }
		   if ( eval("thisForm.USERENTRY10_" + h) != null ) {
					strADDTLINFO += "; Line 10: " + eval("thisForm.USERENTRY10_" + h + ".value");
		   }
	
		   if (strADDTLINFO != null) {
			strADDTLINFO = strADDTLINFO.replace( /\x2F/g,"#SF3SLASH#");
		   }
		   strSLINK = eval("thisForm.PAGE_LINK_" + h + ".value");
		   
		   //Add Promotions fields	   
		   if ( eval("thisForm.PROMO_CODE_VALID_" + h) == null )
			  strPROMO_CODE_VALID    = "";
		   else
			  strPROMO_CODE_VALID    = eval("thisForm.PROMO_CODE_VALID_" + h + ".value");
		
		   if ( eval("thisForm.PROMO_CODE_USER_" + h) == null )
			  strPROMO_CODE_USER    = "";
		   else
			  strPROMO_CODE_USER    = eval("thisForm.PROMO_CODE_USER_" + h + ".value");
			  
		   if ( eval("thisForm.PROMO_TYPE_" + h) == null )
			  strPROMO_TYPE    = "";
		   else
			  strPROMO_TYPE    = eval("thisForm.PROMO_TYPE_" + h + ".value");
			  
		   if ( eval("thisForm.PROMO_ADJUST_" + h) == null )
			  strPROMO_ADJUST    = "";
		   else
			  strPROMO_ADJUST    = eval("thisForm.PROMO_ADJUST_" + h + ".value");
			  
		   //add Quantity Discount fields  
		   if ( typeof eval("thisForm.QD_TYPE_" + h) == "undefined" )
			  strQD_TYPE    = "";
		   else
			  strQD_TYPE    = eval("thisForm.QD_TYPE_" + h + ".value");
		   ...
		   for ( i = 1; i <= iNumberOrdered; i++ ) {
			  ...
			  if ( fields[0] == strID_NUM &&
				 ...
				 SetCookie(strNewOrder, dbUpdatedOrder, null, "/");
				 //notice = strQUANTITY + " " + strNAME + strAdded;
				 notice = "Products successfully" + strAdded;
				 break;
			  }
		   }
	...
	//all products in table now added the cart
...
//---------------------------------------------------------------------||
function changeprice( form_to_use, table_row, sell_exVAT, qd_flag ) {
		if (typeof form_to_use != 'undefined')
		{
		    ...
		    {
		    //ie
		        if (SearchChildForPrice(form_to_use.form.childNodes[x], form_to_use, levelarray, 0, table_row, sell_exVAT, qd_flag))
		        {
		                break;
		        }
		    }
...
//---------------------------------------------------------------------||
function SearchChildForPrice ( FormElement, form_to_use, levelarray, level, table_row, sell_exVAT, qd_flag ) {

    var responce = false; 
    //is it me
    if (typeof FormElement != 'undefined'){
		var price_name = "price"; var Gprice_name = "Gprice";
		if(table_row != -1){
			price_name = "price_" + table_row; Gprice_name = "Gprice_" + table_row;
		}
	    if (FormElement.className == price_name){
    	    UpdatePriceElement( FormElement, form_to_use, table_row, sell_exVAT, qd_flag );
    	}
    	if (FormElement.className == Gprice_name){
    	    UpdateGrossPriceElement( FormElement, form_to_use, table_row, sell_exVAT, qd_flag );
    	    return true;
    	}
    	...
    	for ( y=0; y < FormElement.childNodes.length; y++ )
    	    {
    		 ...
    		 levelarray[level] = y
    		 responce = SearchChildForPrice( FormElement.childNodes[y], form_to_use, levelarray, level + 1, table_row, sell_exVAT, qd_flag);
    		 //restore y
...
//---------------------------------------------------------------------||	
function UpdatePriceElement ( Element, form_to_use, table_row, sell_exVAT, qd_flag ){
	var xx = new Array();
	if(table_row != -1){
		//tabular inpt
		if ( eval("form_to_use.form.ADDITIONALINFO_" + table_row) != null ) {
			//if (form_to_use.form.ADDITIONALINFO.selectedIndex > 0) {
				xx = eval("form_to_use.form.ADDITIONALINFO_" + table_row + "[form_to_use.form.ADDITIONALINFO_" + table_row + ".selectedIndex].value.split('^')");
				//note that for the php package form.PRICE is now the VAT inclusive price so the new ex VAT price = new adjusted price - VAT

				if(sell_exVAT == "N"){
					var origPRICE = eval("form_to_use.form.PRICE_" + table_row + ".value");
					var newprice =  parseFloat(origPRICE) + parseFloat( xx[1] );
				}else{
					var origPRICE = eval("form_to_use.form.PRICE_EXVAT_" + table_row + ".value");
					var newprice =  parseFloat(origPRICE) + parseFloat( xx[1] );
				}
			//}
		}
			
		if ( eval("form_to_use.form.ADDITIONALINFO2_" + table_row) != null ) {
			//if (form_to_use.form.ADDITIONALINFO3.selectedIndex > 0) {		            
				xx = eval("form_to_use.form.ADDITIONALINFO2_" + table_row + "[form_to_use.form.ADDITIONALINFO2_" + table_row + ".selectedIndex].value.split('^')");
		...
		if (newprice) {
			if(sell_exVAT == "N"){
				...
				newprice = ((newprice * 10000) / ((10000 + eval("form_to_use.form.TAX_" + table_row + ".value") * 100)/100))/100;   
			}
			Element.innerHTML = moneyFormat(newprice);
		}
		
		if (updatehiddenpricefield){
	    	form_to_use.form.getElementById("PRICE_" + table_row).value = moneyFormat(newprice);
		}
	}else{
		if ( form_to_use.form.ADDITIONALINFO != null ) {
			//if (form_to_use.form.ADDITIONALINFO.selectedIndex > 0) {
				...
				if(sell_exVAT == "N"){
					...
					var origPRICE = form_to_use.form.PRICE_EXVAT.value;
					var newprice =  parseFloat(origPRICE) + parseFloat( xx[1] );
				}
			//}
		}
...
// PURPOSE:     Updates the Price Element                              ||
//---------------------------------------------------------------------||	
function UpdateGrossPriceElement ( Element, form_to_use, table_row, sell_exVAT, qd_flag ){
	var xx = new Array();
	if(table_row != -1){
		if ( eval("form_to_use.form.ADDITIONALINFO_" + table_row) != null ) {
			//if (form_to_use.form.ADDITIONALINFO.selectedIndex > 0) {
...
//              Also looks in quantity discount table (if one is setup)||
//              and writes the new price (if applicable)               ||
//---------------------------------------------------------------------||
function ChangeQuantity(OrderItem,NewQuantity, sell_exVAT) {
   if ( isNaN(NewQuantity) ) {
      ...
      fields[0] = database.substring( 0, Token0 );         		   // ID_Num
      fields[1] = database.substring( Token0+1, Token1 );			// Quantity
      fields[2] = database.substring( Token1+1, Token2 );			// Price (VAT inclusive)
      fields[3] = database.substring( Token2+1, Token3 );			// Name
      fields[4] = database.substring( Token3+1, Token4 );			// Shipping
	  fields[5] = database.substring( Token4+1, Token5 );			// Tax eg. 20.00(%)
	  fields[6] = database.substring( Token5+1, Token6 );			// Weight
	  fields[7] = database.substring( Token6+1, Token7 );			// Tax Exemption Flag
      fields[8] = database.substring( Token7+1, Token8 );	// Additional Information
	  fields[9] = database.substring( Token8+1, Token9 );	// page link
	  fields[10] = database.substring( Token9+1, Token10 );	// SKU code
	  fields[11] = database.substring( Token10+1, Token11 );	// Currency Symbol
	  MonetarySymbol = fields[11];
	  //Promotions
	  fields[12] = database.substring( Token11+1, Token12 );				// Promotion Code - Valid
	  fields[13] = database.substring( Token12+1, Token13 );				// Promnotion Code - User Keyed
	  fields[14] = database.substring( Token13+1, Token14 );				// Promotion Type
	  fields[15] = database.substring( Token14+1, Token15 );				// Promotion Adjustment
	  //Quantity Discounts
	  fields[16] = database.substring( Token15+1, Token16 );				// Quantity Discount Type
	  fields[17] = database.substring( Token16+1, Token17 );				// Price - EXVat
	  fields[18] = database.substring( Token17+1, database.length );		// Base Price for Quantity Discount calculations
	  
	  //alert(fields[2] + "/" + fields[5]);
	  strPrice = fields[2]; strPrice_exVAT = fields[17];
	  if(fields[16] != ""){
	  	strPrice = getCartQDPrice(fields[16], fields[0], NewQuantity, fields[2], fields[17], fields[18], fields[5]);
		if(sell_exVAT == "Y"){
			//strip off VAT for new exVAT price - 24/09/2103
			strPrice_exVAT = precise_round(((parseFloat(strPrice) * 10000) / ((10000 + parseFloat(fields[5]) * 100)/100))/100, 2);
		}
	  }
...
// PARAMETERS:  Order Number to Add a Promotion Code                   ||
// RETURNS:     Null                                                   ||
// PURPOSE:     Adds a Promotion Code to an item in the shopping cart  ||
//---------------------------------------------------------------------||
function AddPromotionCode(OrderItem,PromoCode, ValidPromo, strErrPromo, flag) {
	if ( PromoCode != ValidPromo ) {
      ShowAlert( strErrPromo, "red" );
	  document.getElemetById("promo-code").value ="";
   	} else {
		NewOrder = "Order." + OrderItem;
		database = "";
		database = GetCookie(NewOrder);
...
//---------------------------------------------------------------------||
// FUNCTION:    Update Promotions                                      ||
// PARAMETERS:  Null                                                   ||
// RETURNS:                         								   ||
// PURPOSE:     Updates Promotion elements within cookie         	   ||
//---------------------------------------------------------------------||
function UpdatePromotions() {
   ...
   keyed_codes = new Array; not_keyed_codes = new Array();
   for ( i = 1; i <= iNumberOrdered; i++ ) {
	  //FIRST PASS down the cookie the purpose of which is simply to build an array of all valid promotion codes already keyed in
      NewOrder = "Order." + i;
      database = "";
      database = GetCookie(NewOrder);
      ...
      fields = new Array;
	  //Unpack Promotions Only
	  fields[12] = database.substring( Token11+1, Token12);  			//Promotion Code - Valid
	  if(fields[12] != ""){active_promotion = 1;}
	  fields[13] = database.substring( Token12+1, Token13 ); 		   //Promotion Code - User Keyed

	  if(fields[12] != "" && fields[13] == fields[12]){
		  //the user has previously successfully keyed in a promotion code against this product
		  keyed_codes.push(fields[12]);
	  }
	  if(fields[12] != 0 && fields[13] == ""){
		 //build an array of valid promotion codes yet to be keyed in by user
		 not_keyed_codes.push(fields[12]);
	  }
   }
   var second_pass = 0;
   if(not_keyed_codes.length > 0){
	   for( i = 0; i < not_keyed_codes.length; i++){
		  //now check to see whether we need a second pass or not ie. have any of the valid codes not yet keyed against various products already been keyed in
		  //against other products on the same promotion
		  if(keyed_codes.indexOf(not_keyed_codes[i]) >= 0){
			  second_pass = 1;
		  }
	   }
   }
   if(second_pass == 1){
	   for ( i = 1; i <= iNumberOrdered; i++ ) {
		  //SECOND PASS down the cookie.
		  //If a product contains a valid promotion code then that product is on promotion
		  //If that same promotion code is found in the array just built above then the user has already correctly keyed this code in against at least one other product
		  //in the cart on the same promotion.
		  //Once a user has successfully keyed the promotion code against a single product on promotion then he is deemed to have keyed that same code in against all
		  //products within the cart on the same promotion - saves him having to key in the same promotion code more than once.
		  //This is generally for use where a user may have keyed in say 10 products on a single category promotion and obviously doesn't want to have to key in
		  //the same code 10 times.
		  //putting fields[13] = fields[12] will simulate the user keying in a valid code against this line
		  NewOrder = "Order." + i;
		  database = "";
		  database = GetCookie(NewOrder);
		  ...
		  if(keyed_codes.length >= 0 && keyed_codes.indexOf(fields[12]) >= 0){
			  //set keyed promotion code to equal the valid code
			  AddPromotionCode(i, fields[12], fields[12], "", 0);  
		  }
	   }
   }
...
// PURPOSE:     Draws current cart product table on HTML page          ||
//---------------------------------------------------------------------||
function ManageCart(sell_exVAT) {
   //sell_exVAT="N";
   var active_promotion = UpdatePromotions();
	
   var iNumberOrdered = 0;    //Number of products ordered
   var fTotal         = 0;    //Total cost of order
   var fTax           = 0;    //Tax amount
   var fTotal_exVAT   = 0;    //Total cost of order (exVAT)
   var fTax_exVAT     = 0;    //Tax amount (exVAT)
   ...
   if ( bDisplay )
      strOutput = "<table class=\"nopcart\"><tr>" +
				  ...
				  if(active_promotion == 1){
			      	strOutput += "<td class=\"nopheader\"><strong>"+strPromLabel+"</strong></td>"; 
			 	  }
                  strOutput += "<td class=\"nopheader\"><strong>"+strQLabel+"</strong></td>" +
   ...
   for ( i = 1; i <= iNumberOrdered; i++ ) {
	  ...
	  var strPrice_inc_promo = fields[2]; //this will become the normal selling price with any subsequent promotion applied
	  var strPrice_inc_promo_exVAT = fields[17]; //this will become the normal exVAT price with any subsequent promotion applied
	  if(fields[16] != ""){
		  //Quantity Discount has been applied so we need to calculate the selling and exVAT prices
		  var strPrice_inc_promo = getCartQDPrice(fields[16], fields[0], fields[1], fields[2], fields[17], fields[18], fields[5]);
		  var strPrice_inc_promo_exVAT = precise_round(((parseFloat(strPrice_inc_promo) * 10000) / ((10000 + parseFloat(fields[5]) * 100)/100))/100, 2);
	  }
	  if(fields[13] == fields[12]){
		  //user has keyed in a valid promotion code
		  strPromoCode = fields[13];
		  switch (fields[14]){
				case "1":
					//Percentage discount Promotion
					strPrice_inc_promo =((parseFloat((fields[2]) * 100) * (100 + parseFloat(fields[15]))) + 5)/10000;
					if(sell_exVAT == "Y"){
						strPrice_inc_promo_exVAT =((parseFloat((fields[17]) * 100) * (100 + parseFloat(fields[15]))) + 5)/10000;
					}
					break;
				case "2":
					//Straight Price Adjustment Promotion
					strPrice_inc_promo = parseFloat(fields[2]) + parseFloat(fields[15]) ;
					if(sell_exVAT == "Y"){
						//Note that if the system is flagged as sell using exVAT price then any promotion adjustments will have been entered as exVAT adjustments
						//strPrice_inc_promo_exVAT = parseFloat(fields[17]) + parseFloat(fields[15]);
						strPrice_inc_promo_exVAT = precise_round(((parseFloat(strPrice_inc_promo) * 10000) / ((10000 + parseFloat(fields[5]) * 100)/100))/100, 2);
					}
					break;
				default:  
		  }
      ...
      fTotal     += (parseInt(fields[1]) * parseFloat(strPrice_inc_promo) );
	  ...
	  fTax			 += Math.round(parseInt(fields[1]) * CalculateTax( productObject, userZone, strPrice_inc_promo )*100)/100; //fTax        = (fTotal * TaxRate);
	  strTotal    = moneyFormat(fTotal);
      strTax      = moneyFormat(fTax);
	  
	  if(sell_exVAT == "Y"){
		  //Shopping Cart now to show exVAT item price + subtotals with add-on VAT so override above calculated fTax and fTotal - 24/09/2013
		  fTax_exVAT += (parseInt(fields[1]) * (parseFloat(strPrice_inc_promo) - parseFloat(strPrice_inc_promo_exVAT)));
		  fTotal_exVAT += (parseInt(fields[1]) * (parseFloat(strPrice_inc_promo_exVAT)) );
		  strTax      = moneyFormat(fTax_exVAT);
		  strTotal    = moneyFormat(fTotal_exVAT);
	  }
...
//ADS
  ...
  //remove link       strOutput += "<tr><td class=\"nopentry\"><a href='" + fields[0] + ".htm'>"  + fields[0] + "</a></td>";
         ...
         if ( fields[8] == "" )
			...
			//Promotions Box in here
			if(active_promotion == 1){
				if(fields[12] != ""){
				   //strOutput += "<tr><td class=\"nopentry nopprod\"><input id=\"promo-code\" type=\"text\" size=\"10\" value=\"" + fields[13] + "\" onchange=\"AddPromotionCode("+i+", this.value, fields[12], strErrPromo);\" \></td></tr>";
				   strOutput += "<td class=\"nopentry nopprod\"><input id=\"promo-code\" type=\"text\" size=\"10\" value=\"" + fields[13] + "\" onchange=\"AddPromotionCode("+i+", this.value, '" + fields[12] + "', strErrPromo, 1);\" \></td>";
	  
				}else{
				   strOutput += "<td>&nbsp;</td>";
				}
			}
         	strOutput += "<td class=\"nopentry\"><input type=text name=Q size=2 value=\"" + fields[1] + "\" onchange=\"ChangeQuantity("+i+", this.value, '" + sell_exVAT + "');\" class=\"nopquantity\" ></td>";
			if(sell_exVAT == "Y"){
        		strOutput += "<td class=\"nopentry\">"+ MonetarySymbol + (moneyFormat(strPrice_inc_promo_exVAT)) + "/ea</td>"; //item price ex-VAT - 24/09/2013
			}else{
				strOutput += "<td class=\"nopentry\">"+ MonetarySymbol + moneyFormat(strPrice_inc_promo) + "/ea</td>";
			}
		 ...
		 //strOutput += "<tr><td class=\"nopentry nopprod\"><input name=\"promo-code\" type=\"text\" size=\"10\" value=\"" + 	moneyFormat(strPrice_inc_promo) + "\" onchange=\"AddPromotionCode("+i+", this.value);\" \></td></tr>";
      ...
      if ( HiddenFieldsToCheckout ) {
         ...
         strOutput += "<input type=hidden name=\"" + OutputItemPrice     + strFooter + "\" value=\"" + strPrice_inc_promo + "\">";
		 ...
		 strOutput += "<input type=hidden name=\"" + OutputItemPromoCode  + strFooter + "\" value=\"" + fields[13] + "\">";         //Promotions Code
      }
   ...
   if ( bDisplay ) {
	  ...
	  if(sell_exVAT == "Y"){
		  //Shopping Cart now to show exVAT item price + subtotals with add-on VAT so override above calculated fTax and fTotal - 24/09/2013
		  fTax_exVAT += CalculateShippingTax(fShipping, userZone);
		  strTax      = moneyFormat(fTax_exVAT);
	  }
      ...
      if ( (DisplayTaxRow || sell_exVAT == "Y") || TaxByRegion ) {
         if ( TaxByRegion ) {
            ...
            if (DisplayTaxRow == false && sell_exVAT == "N"){
                fTax        = 0;
                strTax      = moneyFormat(fTax);
            }
            strOutput += "<tr><td class=\"noptotal noptax\" colspan=2><strong>"+strTAX+"</strong></td>"; // display tax position was 4
	  ...
	  if(sell_exVAT == "Y"){
		  //Shopping Cart now to show exVAT item price + subtotals with add-on VAT as standard - basketTotal now includes tax - 24/09/2013
...
/*function ShowAlert(sMessage, lShowRed ) {
	//jquery message box
	var innerHtml = "<span class='ui-icon ui-icon-alert' style='float: left; margin: 0 7px 20px 0;'></span>";
        innerHtml = innerHtml + sMessage;
...
function submitbutton(theForm, checkout){
var userZone		= GetCookie("zone");
if (ValidateCart(theForm) == false){
return false;
}
if (DisplayShippingRow == false){
	if(checkout == 1){
		theForm.action='http://dev.shopfitter.com/checkout/sf4checkout.cfm';
		theForm.submit();
	}else{
		//google-analytics active - submit to checkout within cart.php
		return true;
	}
}else{
	if ( userZone == null){
			...
			if(checkout == 1){
				theForm.action='http://dev.shopfitter.com/checkout/sf4checkout.cfm';theForm.submit();
			}else{
				//google-analytics active - submit to checkout within cart.php
				return true;
			}
...
//              checkout.                                              ||
//---------------------------------------------------------------------||
function SFCheckoutCart(sell_exVAT) {
   var iNumberOrdered = 0;    //Number of products ordered
   var fTotal         = 0;    //Total cost of order
   var fTax           = 0;    //Tax amount
   var fShipping      = 0;    //Shipping amount
   ...
   for ( i = 1; i <= iNumberOrdered; i++ ) {
	  ...
	  fields[9] = database.substring( Token8+1, Token9 ); 			   //page link
	  fields[10] = database.substring( Token9+1, Token10 ); 			 //SKU code
	  fields[11] = database.substring( Token10+1, Token11 ); 			//CurrencySymbol
	  MonetarySymbol = fields[11];
	  fields[12] = database.substring( Token11+1, Token12 );			// Promotion Code - Valid
	  fields[13] = database.substring( Token12+1, Token13 );			// Promotion Code - User Keyed
	  fields[14] = database.substring( Token13+1, Token14 );			// Promotion Type
	  ...
	  if(fields[13] == fields[12]){
		  ...
		  switch (fields[14]){
				case "1":
					//Percentage discount Promotion
					//strPrice_inc_promo =((parseFloat((fields[2]) * 100) * (100 + parseFloat(fields[15]))) + 50)/10000;
					//strPrice_inc_promo = moneyFormat((((Math.round(parseFloat(fields[2]) * 100)) * (100 + parseFloat(fields[15]))) + 50)/10000);
					//strPrice_inc_promo = moneyFormat((((parseFloat(fields[2] * 100)) * (100 + parseFloat(fields[15]))) + 50)/10000);
					strPrice_inc_promo = moneyFormat(((parseFloat((fields[2]) * 100) * (100 + parseFloat(fields[15]))) + 5)/10000);
					break;
				case "2":
					//Straight Price Adjustment Promotion
					strPrice_inc_promo = parseFloat(fields[2]) + parseFloat(fields[15]) ;
      ...
      //fTax		 += parseInt(fields[1]) * CalculateTax( GetProduct( fields[0] ), userZone );
      //fTax		 += Math.round(parseInt(fields[1]) * CalculateTax( GetProduct( fields[0] ), userZone, fields[2] )*100)/100;
	  fTax		 += Math.round(parseInt(fields[1]) * CalculateTax( productObject, userZone, strPrice_inc_promo )*100)/100;
      }
	  fVAT = parseFloat(strPrice_inc_promo) - (parseFloat(strPrice_inc_promo) * 100)/(10000 + (productObject.tax * 100)) * 100;
	  fVAT = Math.round(fVAT * 100)/100;
	  tVAT = tVAT + (parseInt(fields[1]) * fVAT);
      strTotal    = moneyFormat(fTotal);
      strShipping = moneyFormat(fShipping);
      ...
      if ( PaymentProcessor != '' ) {
		 ...
		 priceExVAT = strPrice_inc_promo;
		 if (ShippingInCheckout == true) {priceExVAT = parseFloat(strPrice_inc_promo) - fVAT;}
         strOutput += "<input type=hidden name=\"" + OutputItemId        + strFooter + "\" value=\"" + fields[0] + "\">";
         strOutput += "<input type=hidden name=\"" + OutputItemQuantity  + strFooter + "\" value=\"" + fields[1] + "\">";
         strOutput += "<input type=hidden name=\"" + OutputItemPrice     + strFooter + "\" value=\"" + priceExVAT + "\">";
         strOutput += "<input type=hidden name=\"" + OutputItemName      + strFooter + "\" value=\"" + fields[3] + "\">";
		 ...
		 strOutput += "<input type=hidden name=\"" + OutputItemSKU        + strFooter + "\" value=\"" + fields[10] + "\">"; 		  //SKU Code
		 strOutput += "<input type=hidden name=\"" + OutputItemPromoCode  + strFooter + "\" value=\"" + fields[13] + "\">";         //Promotions Code
		 strOutput += "<input type=hidden name=\"" + OutputItemDebug      + strFooter + "\" value=\"" + mk2 + "\">"; //Programmer Debug
         if (taxexempt != "true")
            {
            //strOutput += "<input type=hidden name=\"" + OutputItemTaxRate + strFooter + "\" value=\"" + (GetTaxRate( GetProduct( fields[0] ), userZone)) + "\">";
			strOutput += "<input type=hidden name=\"" + OutputItemTaxRate + strFooter + "\" value=\"" + (GetTaxRate( productObject, userZone)) + "\">";
