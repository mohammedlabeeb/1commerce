function add_table_row(){
	var table = document.getElementById("price_cat_discounts");
...
//	}
	...
	for(var i = 0; i < table_rows.length; i++){
		...
		cells[0].childNodes[0].setAttribute('name', "PCAT_" + i);   //quantity input box
	...
	if(table != null){
		...
		for(var i = 0; i < table_rows.length; i++){
			...
			if(i < table_rows.length - 1){
				...
				var valid = false;
				valid = check_for_empty_field(cells[0].childNodes[0], "Please enter a Price Category Code against row " + (i + 1));
				if(valid == true){
