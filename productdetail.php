...
if(isset($_POST['CREATE_REVIEW'])){
	/* Before anything else block obvious spammers trying to inject email headers */
	$pattern = "/\n|\r|\t|%0A|%0D|%08|%09/";
	if (preg_match($pattern,$_POST['RV_AUTHOR']) || preg_match($pattern,$_POST['RV_TITLE'])){
		$message .= "Potential Spam characters found in headers - Review Not Created - please contact Shopfitter";
		$warning = "red";
	}else{
                ...
                echo "<div id=\"reviews\">";
					...
					echo "<div itemprop=\"aggregateRating\" itemscope=\"\" itemtype=\"http://schema.org/AggregateRating\">";
						...
						if($star_rating > 0){
							...
							echo "<input name=\"CREATE_REVIEW\" type=\"button\" value=\"Be the first to review this product\" onclick=\"show_review_form();\" class=\"small-button\" />";
						}
					echo "</div>";
                    ?>
                    <form id="review_form" action="<?php $pagePath ?>" METHOD="post" onsubmit="return validate_review();">
                        <table>
                            ...
                            <tr>
                                <td>Rating:</td>
                                <td>
                                    <select name="RV_RATING" class="review" style="width: 100px; background-color: #fff9c2;">
                            ...
                            <!-- ADD CAPTCHA -->
                            <?php 
							require_once('includes/captcha_configuration.php'); 
							if(isset($custom_captcha) && $custom_captcha == "yes"){
								echo '
								<tr>
									<td>
										&nbsp;
										<input name="HONEYPOT" class="review" type="hidden" value="">
									</td>
								</tr>
								<tr>
								   <td valign="top">
...
<!-- END OF ADDITIONAL PRODUCTS -->
	...
	<div class="breadcrumb-holder">
    	<?php
		//generate quick menu - scan down the tree code generating entries against each category node
		$fstart = 0;
		$fend = 999;
		while ($fend > 0){
			//echo $tree . "<br/>";
			...
			if($fend > 0){
				...
				//must be the last category within the tree node ie. the current parent
				$node = $tree;
				$menucat = getParentFromTree($node);
				echo "<span class=\"breadcrumb\"><a href=\"/" . urlencode(html_entity_decode($menucat->CA_NAME, ENT_QUOTES)) . "/" . $node . ".htm" . "\">" . html_entity_decode($menucat->CA_NAME,ENT_QUOTES) . "</a></span> &gt; ";
				//also add the current product page but no point in linking to the same page
				echo "<span class=\"breadcrumb-here\">" . html_entity_decode($product->PR_NAME) . "</span>";
			}
			$fstart =$fend + 1;
		}
		?>    
